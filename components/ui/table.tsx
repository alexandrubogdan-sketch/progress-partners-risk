...
export const Table = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="w-full overflow-auto min-w-[248px] p-6 rounded-lg relative border border-gray-alpha-400 bg-background-100">
      <table className="w-full border-collapse text-sm font-sans text-gray-1000">
        {children}
      </table>
    </div>
  );
...
Table.Header = ({ children }: { children: React.ReactNode }) => {
  return (
    <thead className="border-b border-gray-alpha-400">{children}</thead>
  );
};
  ...
  return (
    <>
      ...
      <tbody
        ...
        }${interactive ? " [&_tr:hover]:bg-gray-100 dark:[&_tr:hover]:bg-white/5" : ""}`}
      >
        {children}
      </tbody>
    </>
...
Table.Head = ({ children }: { children: React.ReactNode }) => {
  return (
    <th className="h-10 px-2 align-middle font-medium text-left last:text-right text-gray-900 whitespace-nowrap">
      {children}
    </th>
  );
};
...
Table.Cell = ({
  ...
  return (
    <td
      className={`px-2 py-2.5 align-middle last:text-right ${className || ""}`}
      colSpan={colSpan}
    >
      {children}
    </td>
